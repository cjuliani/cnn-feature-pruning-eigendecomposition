...
def conv2d(input,filters,kernel_size,stride,padding,name,act=tf.nn.relu):
	return tf.keras.layers.Conv2D(filters, kernel_size, strides=stride, padding=padding, activation=act,
		kernel_initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.05), bias_initializer=tf.constant_initializer(value=0.1),
		kernel_regularizer=tf.keras.regularizers.l1(l=0.1), bias_regularizer=tf.keras.regularizers.l1(l=0.1),
		activity_regularizer=tf.keras.regularizers.l1(l=0.1),name=name)(input)
...
def build_model(input,is_training,drop_rate=0,f=32,krl=3,pad="same"):
	'''
	Create U-Net with
	...
	:param is_training: False for inference
	''' 
	...
	conv94_0_1 = conv2d(input=conv92, filters=2, kernel_size=1, stride=1, padding=pad, name='conv94_0_1', act=None)
